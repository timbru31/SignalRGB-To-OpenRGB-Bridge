...
class Device {
    ...
    constructor(buffer, deviceId, protocolVersion) {
        ...
        if (protocolVersion >= 5) {
            ...
            let flagcheck_str = this.flags.toString(2);
            let flagcheck = Array(flagsArray.length - flagcheck_str.length).concat(flagcheck_str.split("")).reverse();
            flagcheck.forEach((el, i) => {
                if (el == "1")
                    this.flagList.push(flagsArray[i]);
            });
        }
    }
}
...
function readModes(buffer, modeCount, offset, protocolVersion) {
    ...
    for (let modeIndex = 0; modeIndex < modeCount; modeIndex++) {
        ...
        let flagcheck_str = modeFlags.toString(2);
        let flagcheck = Array(flags.length - flagcheck_str.length).concat(flagcheck_str.split("")).reverse();
        flagcheck.forEach((el, i) => {
            if (el == "1")
                flagList.push(flags[i]);
        });
...
function readZones(buffer, zoneCount, offset, protocolVersion) {
    ...
    for (let zoneIndex = 0; zoneIndex < zoneCount; zoneIndex++) {
        ...
        let zoneFlags = undefined;
        let flagList = undefined;
        if (protocolVersion >= 5) {
            zoneFlags = buffer.readUInt32LE(offset);
            offset += 4;
            flagList = [];
            const flags = ["resizeEffectsOnly"];
            let flagcheck_str = zoneFlags.toString(2);
            let flagcheck = Array(flags.length - flagcheck_str.length).concat(flagcheck_str.split("")).reverse();
            flagcheck.forEach((el, i) => {
                if (el == "1")
                    flagList.push(flags[i]);
